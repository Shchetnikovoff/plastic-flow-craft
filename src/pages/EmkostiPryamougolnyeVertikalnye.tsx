...
const VertRectProductTable = ({ selectedMaterial, selectedColor, onMaterialChange, onColorChange }: VertRectProductTableProps) => {
  ...
  const buildArticle = useCallback((volume: number) => {
    ...
    return `СЗПК.ЕПОВ.${matCode}${colorPart}.${volume}`;
  }, [matCode, selectedColor, hasMultipleColors]);

  const exampleArticle = buildArticle(pryamougolnyeVertikalnyeProducts[0].volume);
  ...
  const segments: ArticleSegment[] = useMemo(() => {
    const segs: ArticleSegment[] = [
      ...
      { value: "ЕПОВ", label: "Тип", desc: "Прямоугольная вертикальная в обрешётке" },
    ...
    segs.push({ value: String(pryamougolnyeVertikalnyeProducts[0].volume), label: "Объём, л", desc: "Объём в литрах" });
    return segs;
  }, [matCode, selectedColor, hasMultipleColors]);
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-slate-50">
                ...
                <TableHead className="font-semibold text-slate-700">Глубина, мм</TableHead>
            ...
            <TableBody>
              {pryamougolnyeVertikalnyeProducts.map((p) => {
...
const EmkostiPryamougolnyeVertikalnye = () => {
  ...
  return (
    <CorporatePageShell
      ...
      title="Прямоугольные вертикальные"
      accentWord="ёмкости в обрешётке"
      subtitle="Вертикальная компоновка для минимального занимаемого пространства при высоких потолках. Объём от 500 до 25 000 литров."
      ...
      seo={{
        title: "Прямоугольные вертикальные ёмкости в обрешётке — купить",
        description: "Вертикальные прямоугольные ёмкости из полипропилена и полиэтилена в обрешётке. Объём от 500 до 25 000 л. Производство, доставка, монтаж.",
        keywords: "вертикальные ёмкости, прямоугольные ёмкости, ёмкости в обрешётке, полипропилен",
      }}
    >
...
export default EmkostiPryamougolnyeVertikalnye;

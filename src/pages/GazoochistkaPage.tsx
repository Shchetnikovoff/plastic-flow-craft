    ...
    <>
        ...
        <div className="mx-auto max-w-[960px] px-4 sm:px-6 py-10 sm:py-14">
          <Breadcrumb className="mb-6">
            <BreadcrumbList className="text-slate-400">
              ...
              <BreadcrumbItem><BreadcrumbPage className="text-slate-200">Газоочистка</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold leading-tight mb-4">
            Газоочистное <span className="text-amber-400">оборудование</span> из полимеров
          </h1>
          <p className="text-sm sm:text-base text-slate-300 mb-6 max-w-2xl leading-relaxed">
            Скрубберы, фильтры и каплеуловители — эффективная очистка промышленных газовых выбросов из химически стойких полимеров.
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="rounded-lg border border-slate-800 overflow-hidden bg-slate-800/50"><img src="/images/gazoochistka-hero-1.png" alt="Газоочистное оборудование" className="w-full object-contain" /></div>
            <div className="rounded-lg border border-slate-800 overflow-hidden bg-slate-800/50"><img src="/images/gazoochistka-hero-2.png" alt="Скрубберы и фильтры" className="w-full object-contain" /></div>
          </div>
        </div>
      </section>
      ...
      <main className="mx-auto max-w-[960px] px-4 sm:px-6 py-6 sm:py-8">
        ...
        <section id="opisanie" className="mb-10">
          <h2 className="text-base font-bold text-foreground mb-3 tracking-wide uppercase border-l-4 border-amber-400 pl-3">Системы газоочистки: от проектирования до ввода в эксплуатацию</h2>
        ...
        {category && (
          <section id="katalog" className="mb-10">
            <h2 className="text-base font-bold text-foreground mb-4 tracking-wide uppercase border-l-4 border-amber-400 pl-3">Каталог газоочистного оборудования</h2>
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="flex-1">{(() => {
                ...
                return (<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{category.subcategories.map((sub, i) => { const cc = (<><div className="aspect-[4/3] bg-muted flex items-center justify-center">{sub.image ? <img src={sub.image} alt={sub.name} className="w-full h-full object-contain" /> : <ImageOff className="h-10 w-10 text-muted-foreground/40" />}</div><div className="px-3 py-2.5"><p className="text-xs text-muted-foreground font-semibold">{catIndex}.{i + 1}</p><p className="text-sm font-medium text-foreground group-hover:text-amber-500 transition-colors mt-0.5">{sub.name}</p></div></>); if (sub.externalPath) return <Link key={sub.id} to={sub.externalPath} className="group rounded-lg border border-border bg-card overflow-hidden hover:border-amber-400/60 hover:shadow-lg hover:shadow-amber-500/10 hover:scale-[1.02] transition-all block">{cc}</Link>; return <button key={sub.id} onClick={() => setSelectedSubId(sub.id)} className="group rounded-lg border border-border bg-card overflow-hidden hover:border-amber-400/60 hover:shadow-lg hover:shadow-amber-500/10 hover:scale-[1.02] transition-all text-left">{cc}</button>; })}</div>);
              })()}</div>
            </div>
          </section>
        )}
        ...
        <section id="cta-form" className="mb-10 scroll-mt-20">
          <div className="rounded-xl bg-slate-900 text-white p-6 sm:p-8">
            <h2 className="text-base font-bold text-white mb-2 tracking-wide uppercase">Готовы заказать газоочистное оборудование?</h2>
            <p className="text-sm text-slate-300 mb-5">Оставьте заявку — расчёт стоимости в течение 24 часов.</p>

        ...
        <div className="mx-auto max-w-[960px] px-4 sm:px-6 py-10 sm:py-14">
          <Breadcrumb className="mb-6">
            <BreadcrumbList className="text-slate-400">
              ...
              <BreadcrumbItem><BreadcrumbPage className="text-slate-200">Водоочистка</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold leading-tight mb-4">
            Водоочистное <span className="text-amber-400">оборудование</span> из полимеров
          </h1>
          <p className="text-sm sm:text-base text-slate-300 mb-6 max-w-2xl leading-relaxed">
            Эффективность, долговечность и устойчивость к агрессивным средам. ФФУ, ламельные отстойники, станции дозирования и жироуловители.
          </p>
          <div className="flex flex-wrap gap-3 mb-8">
            ...
            <Button
              variant="outline"
              onClick={() => document.getElementById("produkciya")?.scrollIntoView({ behavior: "smooth" })}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <div key={img.alt} className="rounded-lg border border-slate-800 overflow-hidden bg-slate-800/50 flex items-center justify-center aspect-square">
      ...
      <main className="mx-auto max-w-[960px] px-4 sm:px-6 py-6 sm:py-8">
                  ...
                  return (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {category.subcategories.map((sub, i) => {
                        ...
                        if (sub.externalPath) {
                          return (
                            <Link key={sub.id} to={sub.externalPath} className="group rounded-lg border border-border bg-card overflow-hidden hover:border-amber-400/60 hover:shadow-lg hover:shadow-amber-500/10 hover:scale-[1.02] transition-all text-left block">
                              {cardContent}
                            </Link>
                          );
                        }
                        return (
                          <button key={sub.id} onClick={() => setSelectedSubId(sub.id)} className="group rounded-lg border border-border bg-card overflow-hidden hover:border-amber-400/60 hover:shadow-lg hover:shadow-amber-500/10 hover:scale-[1.02] transition-all text-left">
                            {cardContent}
                          </button>
                        );
                      })}
        ...
        <section id="produkciya" className="mb-10">
          ...
          <Accordion type="multiple" defaultValue={[products[0].title]} className="space-y-2">
            {products.map((prod) => (
              <AccordionItem key={prod.title} value={prod.title} className="rounded-lg border border-slate-200 bg-white px-4">
